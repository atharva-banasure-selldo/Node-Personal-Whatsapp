...
app.post('/messages/send/:userId', async (req, res) => {
    ...
    if (!userId || !number || !message || !sender) {
        return res.status(400).json({ error: 'userID, number, message, and sender are required' });
    }
    console.log("userId:",userId); 
    ...
    if (!state || !state.isAuthenticated) {
        return res.status(400).json({ error: 'WhatsApp client not ready or not authenticated' });
    }
